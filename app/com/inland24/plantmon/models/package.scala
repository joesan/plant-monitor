...
package object models {
  ...
  implicit val powerPlantCfgFormat: Format[PowerPlantConfig] =
    new Format[PowerPlantConfig] {
      def reads(json: JsValue): JsResult[PowerPlantConfig] = {
        val powerPlantTyp =
          PowerPlantType.fromString((json \ "powerPlantType").as[String])
        powerPlantTyp match {
          ...
          case PowerPlantType.RampUpType =>
            for {
              ...
              rampRateInSeconds <- (json \ "rampRateInSeconds")
  ...
  implicit def toPowerPlantRow(cfg: PowerPlantConfig): Option[PowerPlantRow] =
      ...
      case RampUpType =>
        Some(
          PowerPlantRow(
            ...
            maxPower = cfg.maxPower,
            rampRatePower =
              Some(cfg.asInstanceOf[RampUpTypeConfig].rampPowerRate),
            rampRateSecs = Some(
              cfg.asInstanceOf[RampUpTypeConfig].rampRateInSeconds.toSeconds),
            createdAt = DateTime.now(DateTimeZone.UTC),
            updatedAt = DateTime.now(DateTimeZone.UTC)
          ))
      case _ => // TODO Log
  ...
  implicit def toPowerPlantsConfig(
      seqPowerPlantRow: Seq[PowerPlantRow]): PowerPlantsConfig = {
    PowerPlantsConfig(
  ...
  implicit def toPowerPlantConfig(
      powerPlantRow: PowerPlantRow): PowerPlantConfig = {
